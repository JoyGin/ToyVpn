...
//                Log.i(getTag(), "Read the outgoing packet from the input stream, length: " + length);

                if (length > 0) {
                    Log.i(getTag(), "从TUN设备获取数据，length: " + length  + ", sourceIp: " + getSourceIp(bytes) + ", destIp: " + getDestIp(bytes));
...
//                Log.i(getTag(), "Read the incoming packet from the tunnel, length: " + length);
                if (length > 0) {
                    Log.i(getTag(), "从tunnel获取数据，sourceIp: " + getSourceIp(bytes) + ", destIp: " + getDestIp(bytes));
//                    printBytes(packet.array(), length);
                ...
                if (idle) {
                    ...
                    if (lastSendTime + KEEPALIVE_INTERVAL_MS <= timeNow) {
                        Log.i(getTag(), "lastSendTime + KEEPALIVE_INTERVAL_MS <= timeNow");
                        ...
                        Log.i(getTag(), "lastReceiveTime + RECEIVE_TIMEOUT_MS <= timeNow");
...
//                        throw new IllegalStateException("Timed out");
    ...
    private String getSourceIp(byte[] bytes) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < 4; i++) {
            byte b = bytes[i + 12];
            int decimal = Integer.parseInt(byteToString(b), 2);
            sb.append(decimal);
            if (i != 3) {
                sb.append(".");
            }
    ...
    private String getDestIp(byte[] bytes) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < 4; i++) {
            byte b = bytes[i + 16];
            int decimal = Integer.parseInt(byteToString(b), 2);
    ...
    private void printBytes(byte[] bytes, int length) {
        ...
        Log.i(getTag(), "bytes: " + sb);
    }

    private String byteToString(byte b) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < 8; i++) {
            byte tmp = (byte) (b >> ( 7 - i ) & 1);
            sb.append(tmp);
        }
        return sb.toString();
    }
    ...
    private ParcelFileDescriptor configure(String parameters) throws IllegalArgumentException {
        ...
        Log.i("parameters", "parameters: " + parameters);
        Builder builder = mService.new Builder();
